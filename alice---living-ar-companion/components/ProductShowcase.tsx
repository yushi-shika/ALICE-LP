...
export const ProductShowcase: React.FC<ProductShowcaseProps> = ({ theme }) => {
  ...
  const getAriaReply = (userText: string) => {
    ...
    // fallback rotates through canned replies
  ...
  return (
    <section className="min-h-screen flex items-center justify-center relative overflow-hidden bg-slate-50 p-4 md:p-8">
      {/* Background Decor to simulate "focus" mode */}
      <div className={`absolute inset-0 ${theme.bgGradient} opacity-30`} />
      
      <div className="container mx-auto relative z-10 w-full max-w-7xl">
        {/* AR Glasses Viewport */}
        <div className="relative w-full aspect-[16/9] md:aspect-[21/9] rounded-[2rem] md:rounded-[3rem] shadow-xl overflow-hidden bg-transparent group isolate">
            
            {/* 1. Real World Layer (Background) */}
            <div className="absolute inset-0 z-0">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-white/20 via-transparent to-white/5 mix-blend-screen" />
            </div>

            {/* 2. Character Layer (ALICE) - FIXED VISIBILITY */}
            {/* Removed animations that might hide the element. explicitly set z-index to 10. */}
            <div className="absolute bottom-0 right-0 md:right-[10%] h-[90%] w-auto z-10 flex items-end justify-center pointer-events-none">
                 <img 
                     src="/character.png"
                     className="h-full w-auto object-contain object-bottom drop-shadow-[0_10px_30px_rgba(0,0,0,0.5)]"
                     alt="ALICE Character"
                 />
            </div>

            {/* 3. AR HUD Layer (Overlay) */}
            <div className="absolute inset-0 z-20 p-6 md:p-12 flex flex-col justify-between pointer-events-none select-none">
                
                {/* Top HUD: Status Bar */}
                <div className="flex justify-between items-start">
                    {/* Left: Location/Context */}
                    <div className="flex flex-col gap-2">
                         <div className={`bg-slate-900/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 flex items-center gap-3 text-white shadow-lg`}>
                            <Navigation size={14} className="animate-pulse text-sky-400" />
                            <span className="text-[10px] md:text-xs font-mono tracking-widest uppercase text-sky-50">San Francisco</span>
                         </div>
                    </div>
                            ...
                            <div className="w-full max-w-lg space-y-3 flex flex-col">
                                            ...
                                            {msg.sender === 'aria' && (
                                                <>
                                                  ...
                                                  <div className="relative z-10 space-y-2">
                                                    <span className="text-cyan-50 font-semibold text-[10px] uppercase block tracking-wider">Aria</span>
                                                    <p className="text-base md:text-lg font-medium text-white/90 leading-relaxed drop-shadow-[0_4px_12px_rgba(0,0,0,0.35)]">{msg.text}</p>
                                                  </div>
                                                </>
                                            )}
                                            {msg.sender === 'user' && (
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    <button
                                      ...
                                      className={`bg-slate-900/80 hover:bg-slate-900 backdrop-blur-md px-6 py-2.5 rounded-full border border-white/10 text-white text-sm font-medium transition-all pointer-events-auto shadow-lg ${ariaFinished ? 'opacity-50 cursor-not-allowed' : 'hover:scale-105'}`}
                                    >
                                        Sure, let's go.
                                    </button>
                                    <button
                                      ...
                                      disabled={ariaFinished}
                                      className={`bg-white/10 hover:bg-white/20 backdrop-blur-md px-6 py-2.5 rounded-full border border-white/20 text-white text-sm font-medium transition-all pointer-events-auto ${ariaFinished ? 'opacity-50 cursor-not-allowed' : 'hover:scale-105'}`}
                                    >
                                        Not now.
                                    </button>
                                </div>
                                ...
                                <div className="flex items-center gap-3 bg-white/10 border border-white/10 rounded-full px-4 py-2 backdrop-blur-xl shadow-lg">
                                    <input
                                      ...
                                      disabled={ariaFinished}
                                      className={`flex-1 bg-transparent text-white placeholder:text-white/50 focus:outline-none text-sm ${ariaFinished ? 'opacity-60 cursor-not-allowed' : ''}`}
                                    />
                                    <button
                                      type="button"
                                      onClick={() => handleSend()}
                                      disabled={ariaFinished}
                                      className={`px-4 py-2 bg-cyan-500 hover:bg-cyan-400 text-white rounded-full text-sm font-semibold transition-colors ${ariaFinished ? 'opacity-50 cursor-not-allowed' : ''}`}
                                    >
                                      Send
                                    </button>
                                </div>
            ...
            {/* Glass Glare Overlay - Adds realism */}
            <div className="absolute -inset-full top-0 block w-1/2 -skew-x-12 bg-gradient-to-r from-transparent via-white to-transparent opacity-10 animate-shine pointer-events-none z-30" />
        </div>
